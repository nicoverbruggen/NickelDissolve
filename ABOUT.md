...
| `nds_debug_force_unsupported` | 0 | **i.MX only, for testing the unsupported path on a REAGL board:** `1` = treat the first reading turn as if it used a non-REAGL waveform, so the mod runs its "device unsupported" flow (writes `determined_unsupported:1`, turns the animation off, shows the toast). Note that it persists `determined_unsupported:1` to the config; to go back, remove *both* keys from the config file |
...
**Officially supported: the modern MediaTek (`hwtcon`) devices.** These are the Kobo Clara BW, Clara Colour, and Libra Colour, and they are what the animation is built and tested for. The i.MX (`mxcfb`) and AllWinner (`sunxi`) code paths are still present, so the mod installs and can run on those devices too, but they are **not officially supported and may not work**. The Reading-settings entry reflects this in three tiers: a supported (`hwtcon`) device shows a plain on/off row; an i.MX (`mxcfb`) device keeps the on/off row but adds a caution that it may not work; anything else shows an *Unsupported* label in place of the toggle. *Tested by author* = personally run on the hardware.
...
So `nds_wf_sweepable` sweeps **only** `GL16`/`GLR16` (greyscale reading turns). That single test skips colour pages, `GC16` flashes, `AUTO`(257), and `A2`/`DU` menu updates, and because those are also the waveforms the home screen and menus use, it doubles as the reader-context guard (returning to the home screen never gets swept). On a mono panel (mxcfb, no CFA) the i.MX gate is stricter still, and for a reason beyond just "is it a reading turn." On-device logs show the Libra 2 ships in two flavours: one board reads with **REAGL(6)** (fast, clean) and an older board with **GLKW16(10)** (a heavier mode that renders band-by-band in ~5 s/turn). Both use AUTO(257)/DU for menus/toolbars and GC16 / full-screen AUTO for flashes. So the i.MX gate sweeps **only REAGL(6)**. That single test rejects chrome and flashes *and* skips the slow boards. When a genuine reading turn (past the gesture gate, full-screen, non-flash) instead carries a non-REAGL reading waveform (GL16/REAGLD/GLKW16), the board can't do the fast refresh: the mod records this once as `determined_unsupported` in the config (so it survives reboots), turns the animation off, marks the device Unsupported in the settings, and shows a one-time toast. Swept strips still set `HWTCON_FLAG_CFA_SKIP` to avoid per-region seams (`nds_debug_cfa_skip`); `nds_debug_sweep_any_waveform:1` bypasses the allowlist for experiments.
