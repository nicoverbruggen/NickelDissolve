...
| key | default | meaning |
|---|---|---|
| `nds_debug_strip_waveform` | 0 = per-device default | **0** uses the per-device band waveform (colour panels: `glkw16` in day mode, `GLKW16` in Dark Mode; else reuse the turn's own waveform; see below). Non-zero forces a raw, **platform-specific** id (hwtcon: 1=DU 3=GL16 4=GLR16 8=glkw16 9=GLKW16; mxcfb: REAGL=6 GC16=2) |
...
| mono (mxcfb) | Libra 2, Clara 2E | reuse the turn's `REAGL` | reuse the turn's waveform |
